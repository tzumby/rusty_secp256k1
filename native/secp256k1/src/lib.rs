...
use secp256k1::{sign, Message, PublicKey, SecretKey, Signature};
...
rustler::rustler_export_nifs! {
    ...
    [
        ...
        ("ec_sign", 2, ec_sign, rustler::SchedulerFlags::DirtyCpu)
    ],
    None
}
...
fn ec_sign<'a>(env: Env<'a>, args: &[Term<'a>]) -> Result<Term<'a>, Error> {
    ...
    if private_key_bin.len() != 32 {
        return Ok((atoms::error(), "Private key is not 32 bytes.").encode(env));
    }
    ...
    let (Signature { s, r }, recid) = sign(&message, &private_key);
...
fn ec_pubkey_tweak_add<'a>(env: Env<'a>, args: &[Term<'a>]) -> Result<Term<'a>, Error> {
    ...
    let mut public_key =
        PublicKey::parse_compressed(&public_key_fixed).expect("Error parsing public key");
