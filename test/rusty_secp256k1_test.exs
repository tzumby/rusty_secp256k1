  ...
  describe "ec_sign/2" do
    ...
    test "correctly signs message with the provided private key", %{
      ...
      assert {
             ...
             } = RustySecp256k1.ec_sign(message, private_key)
    end

    test "fails if private key size < 32 bytes", %{message: message} do
      assert {:error, "Private key is not 32 bytes."} = RustySecp256k1.ec_sign(message, <<1>>)
    end

    test "fails if message size < 32 bytes", %{private_key: private_key} do
      assert {:error, "Message is not 32 bytes."} = RustySecp256k1.ec_sign(<<1>>, private_key)
    end

    test "fails if private_key is not binary", %{message: message} do
      assert {:error, "Private key is not binary."} = RustySecp256k1.ec_sign(message, nil)
    end

    test "fails if message is not binary", %{private_key: private_key} do
      assert {:error, "Message is not binary."} = RustySecp256k1.ec_sign(10, private_key)
    end
  end
end
